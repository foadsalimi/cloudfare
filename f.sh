...
Menu() {
    ...
    read -n1 -p "$opc1" opc
   case $opc in
    1) git clone https://github.com/leitura/testproxy.git; cd testproxy; chmod +x scan; ./scan ;;
    2) curl -sO https://raw.githubusercontent.com/leitura/ip-free/main/iniciar.sh; chmod 777 iniciar.sh 2> /dev/null; ./iniciar.sh 2> /dev/null ;;
    3) git clone https://github.com/leitura/subdomainCDN.git; cd subdomainCDN; chmod +x scan; ./scan ;;
0) clear ; exit ;;
